// Generador de los informes formales de la Entidad "Aliansalud": Excel + PDF.
// Tercera Entidad con modelo propio (después de SOS y Famisanar) — mismo
// criterio: nunca inventar columnas, calcar la plantilla real que el usuario
// compartió ("Informe Grupo Aliansalud *.xlsx", hoja "Grupo", leída con
...
// igual que en la plantilla). A diferencia de SOS/Famisanar, esta plantilla
// trae "Cliente" al final (la razón social completa de la Entidad, útil
// también para el "Señores:" de la carta en PDF).
const COLUMNAS_ALIANSALUD = [
  ["_Id", "id", "id"],
...
export async function generarInformeAliansaludExcel(entidad, procesos){
  const { default: ExcelJS } = await import('exceljs');
  const wb = new ExcelJS.Workbook();
  const ws = wb.addWorksheet("Aliansalud");

  ws.columns = [{width: 5}, ...COLUMNAS_ALIANSALUD.map((c,i) => ({width: ANCHOS[i]}))];

  const headerRow = ws.addRow(["", ...COLUMNAS_ALIANSALUD.map(c => c[0])]);
  headerRow.height = 45;
  ...
  procesos.forEach(p => {
    const valores = COLUMNAS_ALIANSALUD.map(([, campo, tipo]) => {
      const raw = p[campo];
  ...
  a.href = url;
  a.download = `Informe Aliansalud ${hoy}.xlsx`;
